...
PDF渲染的默认技术路径是Playwright + Paged.js + HTML/CSS，详见`references/pdf_rendering.md`。写HTML前必须先执行该文档「视觉体系」章节：建立design token，页面与图表同源取色取字；渲染后逐页截图自检，无大面积空白页、无表格破线、封面无孤行才算完成。执行方可选用其他工具（weasyprint、reportlab、puppeteer截图、KIMI/Claude自带PDF能力等），但必须自行解决A4分页、页眉页脚页码、CJK字体三件事，且同时达到`pdf_rendering.md`中的视觉体系要求与可读性底线。
