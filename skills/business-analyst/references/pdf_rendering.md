...
1. **Token先行**：写HTML前先定一套design token并集中声明在`:root`：色彩（1个主色加success/warning/danger语义色加中性灰阶）、字号阶梯（正文10~10.5pt起步）、间距节奏。本机有现成体系（如`~/.claude/rules/design_template.md`）就直接引用，不要即兴配色。badge、风险标注、图表用色全部从这套token派生，禁止三套颜色各管各的。
